...
namespace Tester.WarehouseInventory
{
   public class MaterialRequisition
   {
      ...
      public async Task Start()
      {
         ...
         {
            frm.Logs("Info", $" Material Requisition step {Step}", $"FAILED -- {ex.Message}");

         }
         finally
         {
            frm.UpdateStatus("Proses  Material Requisition Selesai");
         }
      }
      public async Task AllBlank()
      {
         var Proses = " Material Requisition - Insert - All Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            Step = 1; // Navigasi Menu
            var Menu = await page.XPathAsync("//a[contains(@href, '/Inventory/M3Mr?md')]/span");
            if (Menu.Length > 0)
      ...
      public async Task OriWarehouseBlank()
      {
         var Proses = " Material Requisition - Insert - Original Warehouse Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = " Material Requisition - Insert (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            ...
            Step = 2; // Klik Original Warehouse
            await App.ClickBtn("//label[@title='Gudang Asal']/following-sibling::a");
      ...
      {
         var Proses = " Material Requisition - Read (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = " Material Requisition - Update (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            Step = 1; // Input Name 
            await App.InputData("//input[@name='Mruraian']", "Update");
      ...
      //{
      //   var Proses = " Material Requisition - Delete (Positive)"; frm.UpdateStatus(Proses);
      //   Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
      //   int Step = 0;
      //   try
      //   {
