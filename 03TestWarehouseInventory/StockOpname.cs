...
namespace Tester.WarehouseInventory
{
   public class StockOpname
   {
      ...
      public async Task Start()
      {
         ...
         {
            frm.Logs("Info", $" Stock Opname step {Step}", $"FAILED -- {ex.Message}");

         }
         finally
         {
            frm.UpdateStatus("Proses  Stock Opname Selesai");
         }
      }
      public async Task AllBlank()
      {
         var Proses = " Stock Opname - Insert - All Blank (Negative)"; frm.UpdateStatus(Proses);
      ...
      {
         var Proses = " Stock Opname - Insert (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = " Stock Opname - Read (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = " Stock Opname - Update (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      //{
      //   var Proses = " Stock Opname - Delete (Positive)"; frm.UpdateStatus(Proses);
      //   Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
      //   int Step = 0;
      //   try
      //   {
