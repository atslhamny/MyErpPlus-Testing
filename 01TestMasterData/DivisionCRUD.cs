...
namespace Tester
{
   public class DivisionCRUD
   {
      ...
      public async Task Start()
      {
         ...
         try
         {
            Step = 1; // Division - Insert - All Blank (Negative)
            if (frm.IsStopped) return;
            await AllBlank();

            //Step = 2; // Division - Insert - Code Blank (Negative)
            //if (frm.IsStopped) return;
            //await CodeBlank();

            //Step = 3; // Division - Insert - Name Blank (Negative)
            //if (frm.IsStopped) return;
            //await NameBlank();

            Step = 4; // Division - Insert (Positive)
            if (frm.IsStopped) return;
            await Create();

            //Step = 5; // Division - Read (Positive)
            //if (frm.IsStopped) return;
            //await Read();

            //Step = 6; // Division - Update (Positive)
            //if (frm.IsStopped) return;
            //await Update();

            //Step = 7; // Division - Delete (Positive)
            //if (frm.IsStopped) return;
            //await Delete();
         }
         catch (Exception ex)
         {
            frm.Logs("Info", $"Proses TransactionNote - CRUD step {Step}", $"FAILED -- {ex.Message}");
         }
         finally
         {
            frm.UpdateStatus("Proses TransactionNote - CRUD Selesai");
         }
      }

      public async Task AllBlank()
      {
         var Proses = "Division - Insert - All Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            Step = 1; // Navigasi ke menu
            var Menu = await page.XPathAsync("//a[contains(@href, 'M1Division?md')]/span");
            if (Menu.Length > 0)
      ...
      {
         var Proses = "Division - Insert - Code Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            ...
            Step = 2; // Input Nama
            await App.InputData("//input[@name='Dnama']", "Divisi20");

            Step = 3; // Input Catatan
            await App.InputData("//textarea[@name='Dcatatan']", "Catatan Baru");
      ...
      {
         var Proses = "Division - Insert - Name Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = "Division - Insert (Positive)"; frm.UpdateStatus(Proses); ;
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = "Division - Read (Postive)"; frm.UpdateStatus(Proses); ;
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = "Division - Delete (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
