...
namespace Tester
{
   public class SizeCRUD
   {
      ...
      public async Task Start()
      {
         ...
         try
         {
            Step = 1; // Size - Insert - All Blank (Negative)
            if (frm.IsStopped) return;
            await BlankInsert();

            Step = 2; // Size - Insert - Code Blank (Negative)
            if (frm.IsStopped) return;
            await BlankCode();

            Step = 3; // Size - Insert - Name Blank (Negative)
            if (frm.IsStopped) return;
            await BlankName();

            Step = 4; // Size - Insert (Positive)
            if (frm.IsStopped) return;
            await Insert();

            Step = 5; // Size - Read (Positive)
            if (frm.IsStopped) return;
            await Read();

            Step = 6; // Size - Update (Positive)
            if (frm.IsStopped) return;
            await Update();

            Step = 7; // Size - Delete (Positive)
            if (frm.IsStopped) return;
            await Delete();
         }
         catch (Exception ex)
      ...
      {
         var Proses = "Size - Insert - All Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            Step = 1; // Navigasi ke Menu
            var Menu = await page.XPathAsync("//a[contains(@href, 'M1Size?md')]/span");
            if (Menu.Length > 0)
      ...
      {
         var Proses = "Size - Insert - Code Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            ...
            Step = 2; // input name
            await App.InputData("//input[@name='Snama']", "TEST");
      ...
      {
         var Proses = "Size - Insert - Name Blank (Negative)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = "Size - Insert (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
      ...
      {
         var Proses = "Size - Read (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
            ...
            jam.Stop();
            frm.PASS++;
            frm.Logs("P", "Size - Read", "PASS", jam.ElapsedMilliseconds);
         }
         catch (Exception ex)
         {
            frm.FAILED++;
      ...
      {
         var Proses = "Size - Update (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         { 
            ...
            jam.Stop();
            frm.PASS++;
            frm.Logs("P", "Size - Update", "PASS", jam.ElapsedMilliseconds);
         }
         catch (Exception ex)
         {
            frm.FAILED++;
      ...
      {
         var Proses = "Size - Delete (Positive)"; frm.UpdateStatus(Proses);
         Stopwatch jam = new Stopwatch(); jam.Start(); frm.Test++;
         int Step = 0;
         try
         {
